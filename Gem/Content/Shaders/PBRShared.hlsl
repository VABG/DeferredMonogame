...
float3 ShadePixelTest(float3 lightDirection, float3 viewDirection, float3 lightColor, float4 albedo, float4 normalGloss,
                     float4 specularGlow)
{
    ...
    float3 specularLight = float3(
        specularGlow.xyz * pow(saturate(dot(reflectionVector, viewDirection)), normalGloss.a * 20.0f)) * .5f;
